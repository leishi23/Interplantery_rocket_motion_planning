...
BEGIN_ACADO;                                % Always start with "BEGIN_ACADO". 
    acadoSet('problemname', 'AOC_Final');
    ...
    DifferentialState       obx;              % The differential states 
    DifferentialState       oby;              % The differential states 
    ...
    f.add(dot(p_x) == v_x);
    f.add(dot(p_y) == v_y);
    f.add(dot(v_x) == u_l*cos(theta)-p_x*G1/(p_x^2+p_y^2)^1.5+G2*(obx-p_x)/((obx-p_x)^2+(oby-p_y)^2)^1.5);
    f.add(dot(v_y) == u_l*sin(theta)-p_y*G1/(p_x^2+p_y^2)^1.5+G2*(oby-p_y)/((obx-p_x)^2+(oby-p_y)^2)^1.5);
    f.add(dot(theta) == theta_dot);
    
    f.add(dot(t) == 1);
    f.add(dot(theta_dot)== u_r);
    
    f.add(dot(obx) == -8*sin(0.135*t-pi/4)*0.135);
    f.add(dot(oby) == 8*cos(0.135*t-pi/4)*0.135);
    ...
    % (c) Path constraints
    ocp.subjectTo( 1.5 <= ((p_x-obx)^2+(p_y-oby)^2));
    ...
    ocp.subjectTo('AT_START',obx == 8.0/sqrt(2));
    ocp.subjectTo('AT_START',oby == -8.0/sqrt(2));
